...
const universities = [];
const sortGrads = (array) => {
  for (let i = 0; i < array.length; i++) {
    universities[i] = array[i].university;
  }
...
const contactInfo = [];
const contactSort = (array) => {
  for (let i = 0; i < array.length; i++) {
    contactInfo[i] = `${array[i].first_name} ${array[i].email}`;
  }
}
contactSort(graduates);
...
const lowerCase = animalNames.map(function(animal) {
  animal.animal_name;
});
